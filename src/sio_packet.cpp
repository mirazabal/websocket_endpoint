...
namespace mirazabal
{
	namespace sio
	{
		...
		using namespace std;
		void accept_message(sio_message const& msg, Value& val, Document& doc,
			...
			vector<shared_ptr<const string> >& buffers)
		{
			...
			char frame_char = static_cast<char>(sio_packet::frame_type::frame_message);
		...
		{
			val.SetArray();
			for (vector<sio_message::ptr>::const_iterator it =
				msg.get_vector().begin(); it != msg.get_vector().end(); ++it) {
				Value child;
		...
		{
			val.SetObject();
			for (map<string, sio_message::ptr>::const_iterator it = msg.get_map().begin();
		...
		sio_message::ptr from_json(Value const& value, vector<shared_ptr<const string> > const& buffers)
		{
			if (value.IsInt64())
			{
			...
			{
				sio_message::ptr ptr = array_message::create();
			...
			else if (value.IsObject())
			{
				...
				if (mem_it != value.MemberEnd() && mem_it->value.GetBool()) {
					...
					return sio_message::ptr();
				}
				//real object message.
				sio_message::ptr ptr = object_message::create();
		...
		sio_packet::sio_packet(string const& nsp, sio_message::ptr const& msg, int pack_id, bool isAck) :
			_frame(frame_type::frame_message),
			_type(static_cast<int>(isAck ? type::type_ack : type::type_event) | static_cast<int>(type::type_undetermined)),
			_nsp(nsp),
			_message(msg),
			_pack_id(pack_id),
			_pending_buffers(0)
		...
		sio_packet::sio_packet(type type, string const& nsp, sio_message::ptr const& msg) :
			_frame(frame_type::frame_message),
		...
		sio_packet::sio_packet(sio_packet::frame_type frame) :
			_frame(frame),
			_type(static_cast<int>(type::type_undetermined)),
			_pack_id(-1),
			_pending_buffers(0)
		{
		...
		bool sio_packet::is_binary_message(string const& payload_ptr)
		{
			return payload_ptr.size() > 0 && payload_ptr[0] == static_cast<int>(frame_type::frame_message);
		}

		bool sio_packet::is_text_message(string const& payload_ptr)
		{
			return payload_ptr.size() > 0 && payload_ptr[0] == (static_cast<int>(frame_type::frame_message) + '0');
		}

		bool sio_packet::is_message(string const& payload_ptr)
		{
			return is_binary_message(payload_ptr) || is_text_message(payload_ptr);
		}

		bool sio_packet::parse_buffer(const string &buf_payload)
		{
		...
		bool sio_packet::parse(const string& payload_ptr)
		{
			assert(!is_binary_message(payload_ptr)); //this is ensured by outside
			_frame = (sio_packet::frame_type) (payload_ptr[0] - '0');
			_message.reset();
			_pack_id = -1;
			_buffers.clear();
			_pending_buffers = 0;
			size_t pos = 1;
			if (_frame == frame_type::frame_message) {
				_type = static_cast<int>((sio_packet::type)(payload_ptr[pos] - '0'));
				if (_type < static_cast<int>(type::type_min) || _type > static_cast<int>(type::type_max))
				{
					return false;
				}
				pos++;
				if (_type == static_cast<int>(type::type_binary_event) || _type == static_cast<int>(type::type_binary_ack))
				{
			...
			if (payload_ptr[nsp_json_pos] == '/')//nsp_json_pos is start of nsp
			{
				...
				if (comma_pos == string::npos)//sio_packet end with nsp
				{
			...
			if (_frame == frame_type::frame_message && (_type == static_cast<int>(type::type_binary_event) || _type == static_cast<int>(type::type_binary_ack)))
			{
		...
		bool sio_packet::accept(string& payload_ptr, vector<shared_ptr<const string> >&buffers)
		{
			char frame_char = static_cast<char>(_frame) + '0';
			...
			_type = _type & (~static_cast<int>(type::type_undetermined));
			if (_type == static_cast<int>(type::type_event))
			{
				_type = static_cast<int>(hasBinary ? type::type_binary_event : type::type_event);
			}
			else if (_type == static_cast<int>(type::type_ack))
			{
				_type = static_cast<int>(hasBinary ? type::type_binary_ack : type::type_ack);
			}
			ostringstream ss;
			ss.precision(8);
			ss << _type;
		...
		sio_packet::frame_type sio_packet::get_frame() const
		{
			return _frame;
		}

		sio_packet::type sio_packet::get_type() const
		{
			assert((_type & static_cast<int>(type::type_undetermined)) == 0);
			return static_cast<type>(_type);
		}

		string const& sio_packet::get_nsp() const
		{
			return _nsp;
		}

		sio_message::ptr const& sio_packet::get_message() const
		{
			return _message;
		}

		unsigned sio_packet::get_pack_id() const
		{
			return _pack_id;
		}


		/*void packet_manager::set_decode_callback(function<void(sio_packet const&)> const& decode_callback)
		{
		...
		void packet_manager::encode(sio_packet& pack, encode_callback_function const& override_encode_callback) const
		{
